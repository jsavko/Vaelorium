---
name: execute-plan
description: This skill should be used when the user asks to execute a plan, run a plan, or uses "/execute-plan". Finds the active plan file, works through all tasks in order, runs tests, and archives the plan on completion.
version: 1.0.0
---
...
3. **Commit the changes**:
   - Stage all modified and new files related to the plan (source files, changelog, archived plan)
   - Create a git commit with a message summarising the plan title and goal
   - Use the changelog content to inform the commit message body
