...
async fn migrate_legacy_db(app: &tauri::AppHandle, managed: &ManagedDb) {
    ...
    match db::open_database(&tome_path_str).await {
        Ok(pool) => {
            ...
            // Add to recent tomes
            app_state::add_recent_tome(app, &tome_path_str, "My Campaign", None, None, false);
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Backup (app-global backend) — split across submodules
            commands::backup::config::backup_configure,
            commands::backup::config::backup_disconnect,
            commands::backup::config::backup_status,
