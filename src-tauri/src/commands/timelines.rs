...
#[tauri::command]
pub async fn delete_timeline(managed: State<'_, ManagedDb>, id: String) -> Result<(), String> {
    let pool = db::get_pool(managed.inner()).await?;
    sqlx::query("DELETE FROM timelines WHERE id = ?").bind(&id).execute(&pool).await.map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn update_timeline_event(
    ...
    if let Some(ref v) = title { sqlx::query("UPDATE timeline_events SET title = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
    if let Some(ref v) = date { sqlx::query("UPDATE timeline_events SET date = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
    if let Some(ref v) = description { sqlx::query("UPDATE timeline_events SET description = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
    if let Some(ref v) = end_date { sqlx::query("UPDATE timeline_events SET end_date = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
    if let Some(ref v) = page_id { sqlx::query("UPDATE timeline_events SET page_id = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
    if let Some(ref v) = color { sqlx::query("UPDATE timeline_events SET color = ? WHERE id = ?").bind(v).bind(&id).execute(&pool).await.map_err(|e| e.to_string())?; }
...
#[tauri::command]
pub async fn delete_timeline_event(managed: State<'_, ManagedDb>, id: String) -> Result<(), String> {
    let pool = db::get_pool(managed.inner()).await?;
    sqlx::query("DELETE FROM timeline_events WHERE id = ?").bind(&id).execute(&pool).await.map_err(|e| e.to_string())?;
    Ok(())
}
